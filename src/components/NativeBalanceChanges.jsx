...
function NativeBalanceChanges({ changes }) {
  const theme = useTheme();

  console.log("NativeBalanceChanges received props:", changes);
  ...
  if (!changes || !Array.isArray(changes) || changes.length === 0) {
    return (
      <Paper sx={{ marginTop: 2, marginBottom: 2, padding: 2 }}>
        <Typography variant="h6">Native Balance Changes</Typography>
        <Typography>No native balance changes available.</Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper sx={{ marginTop: 2, marginBottom: 2 }}>
      <Typography variant="h6" sx={{ padding: 2, textAlign: 'center' }}>Native Balance Changes</Typography>
      <TableContainer>
        <Table>
          <TableHead>
            <TableRow>
...
export default NativeBalanceChanges;