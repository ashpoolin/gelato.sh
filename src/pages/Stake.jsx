...
function getUnlocks() {
  ...
  fetch(`${URL}/unlocks`)
    ...
    .then((data) => {
      ...
      dataObj.map((line) => y_data.push((+line.delegated ?? 0))); // + removes quotes, ?? 0 sets null values to 0 
      dataObj.map((line) => y2_data.push((+line.delegated_cumulative ?? 0)));
        ...
        >
          ...
          >
            <AccordionSummary
              ...
              id="panel1a-header"
            >
            <Typography>About Unlocks</Typography>
            </AccordionSummary>
            <AccordionDetails>
              <Typography>
              This chart is generated by taking all stakes accounts, and summing the delegated stake balances of all stake accounts (`solana stakes`) with a `custodian` and `unixTimestamp` which occurs later than the current date/time. While any user may create locked stake accounts, this practice is mostly done by the Solana Foundation, and has historically been the vehicle for distributing tokens and grants that have time locks or performance stipulations.
                <Divider sx={{ marginY: 2 }} />
                <h4>Unlock Schedule by Date</h4>
                <TableContainer component={Paper}>
                  {/* <Table className={classes.table} aria-label="supply snapshot table">*/}
                  <Table aria-label="Unlock Schedule (delegated stake)">
                    <TableHead>
                    <TableRow>
                        <TableCell component="th" scope="row">Date</TableCell>
                        <TableCell component="th" scope="row" align="right">Daily</TableCell>
                        <TableCell component="th" scope="row" align="right">Cumulative</TableCell>
                    </TableRow>
                    </TableHead>
                    <TableBody>
                      {unlockObject.map((item, index) => (
                      <TableRow>
                        <TableCell component="th" scope="row">{new Date(item.date_of_unlock).toISOString().split("T")[0]}</TableCell>
                        <TableCell component="th" scope="row" align="right">{formatNumber(item.delegated)}</TableCell>
                        <TableCell component="th" scope="row" align="right">{formatNumber(item.delegated_cumulative)}</TableCell>
                      </TableRow>
                      ))}
